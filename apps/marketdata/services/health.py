...
def _probe_stock_provider(provider_name: str) -> ProviderHealthResult:
    symbol = "AAPL"
    exists, error_message = _instrument_exists(symbol)
    if error_message:
        return ProviderHealthResult(provider_name, False, error_message)
    if not exists:
...
def _instrument_exists(symbol: str) -> tuple[bool, str | None]:
    try:
        return Instrument.objects.filter(symbol=symbol).exists(), None
    except (OperationalError, ProgrammingError):
        return False, "schema_not_ready: run migrations (python manage.py migrate)"
